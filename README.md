...
Alle Dateien müssen im selben Ordner des GitHub-Repositories liegen.
Als **Main file path** wird `app.py` gewählt.

Nach einem Commit aktualisiert Streamlit die App normalerweise automatisch.
Falls nicht: **Manage app → Reboot app**.

## Enthaltene Korrekturen

- `Employee` ist korrekt als `@dataclass` definiert.
- `employee_status_label()` liegt zentral in `models.py`.
- `views.py` importiert diese Funktion ausdrücklich.
